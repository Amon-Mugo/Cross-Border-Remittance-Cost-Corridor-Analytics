...
def unpivot_cc(df:DataFrame) -> DataFrame:
    ...
    shared_cols=[c for c in df.columns if not c.startswith(CC_PREFIXES)] #this filters
    #out the cc1 and cc2 columns and drop them 
    sides = []
    for prefix in CC_PREFIXES:
        cc_number=int(prefix.strip()[-1]) # this removes the whitespace and the cc number
